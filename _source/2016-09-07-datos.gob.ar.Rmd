...
library(DT)
datatable(organization_list(as = "table")[, c(1, 4)])
...
```{r, warning=FALSE, fig.height=10, fig.width=10, echo = FALSE, message=FALSE}
presupuesto_2016 %>% 
  group_by(jurisdiccion_desc) %>% 
  summarise_at(vars(starts_with("monto")), funs(sum))  %>% 
  gather(tipo_monto, value = valor, starts_with("monto")) ->
  presupuesto_2016_by_jurisdiccion

presupuesto_2016_plot  <- ggplot(
  filter(presupuesto_2016_by_jurisdiccion,
         tipo_monto %in% c("monto_presupuestado", "monto_pagado")), 
         aes(jurisdiccion_desc, valor, fill = tipo_monto)) +
    geom_bar(stat = "identity", position = "identity") +
    scale_x_discrete("Jurisdicción", 
                     labels =
                       str_trunc(presupuesto_2016_by_jurisdiccion$jurisdiccion_desc,
                                 30, "r")) + 
    scale_y_continuous("Monto presupuestado (en millones)", 
                       labels = dollar_format(big.mark = ".", decimal.mark = ",")) +  
  scale_fill_discrete(name = "Montos", 
                 breaks = c("monto_pagado", "monto_presupuestado"),
                 labels = c("Pagado", "Presupuestado")) +
    theme_few() +
